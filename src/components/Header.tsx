...
const Header: React.FC<HeaderProps> = ({ onChange }) => {
    ...
    return (
        <div className="w-full text-base  p-5 text-white h-[25vh]  bg-blue-200 grid grid-cols-2 xl:flex xl:flex-row xl:h-[10vh]  ">
            <div className="w-full flex md:flex-row ">
                <img src={logo} alt="logo" className="w-full h-full xl:w-[30%] md:w-[49vw] " />


                <div className="flex items-center m-auto  xl:flex xl:h-full">
                    {/* <input type="search" className=" md:w-full h-1/2 ml-2 bg-[#ffffff] text-black " placeholder="Пошук на сайті" onChange={(event) => {handleNameChange(event) }} /> */}
                    <div className="search-bar w-[48vw] lg:w-[50vw] lg:h-full xl:w-full ">
            ...
            <div className="flex col-span-2 justify-between	 items-center text-black md:space-x-5 xl:w-[50%]  xl:space-between">
                <Link className="nav new md:mx-5" to={'/about'}>Про сайт</Link>
                <Link className="nav md:mx-5" to={'/gallery'}>Галарея картинок</Link>
                <Link className="nav md:mx-5" to={'/news'}>Новини</Link>
